...
1. **The issuer's discretion over supply.** $rPND is capped on ledger at `MaximumAmount`, enforced by the ledger once outstanding supply reaches it. $PND, as an IOU, has no on-ledger cap at all; the operational trust limit bounds one line, not total issuance. The 100B $PND target is operator policy the ledger will not enforce, verifiable only by reading issuer obligations via `gateway_balances` — [OQ-06](open-questions.md#oq-06). How the two figures relate is undecided — [OQ-21](open-questions.md#oq-21).
