...
source_ref: worktree
source_sha256: df8a05a77d004a15ba68a9a0a338262d0ea4ce0b35cf1af6a620b9e713e7e9cf
title: Overview
url: /protocol/
section: Protocol
synced: 2026-09-16
...
The issuance toolkit signs transactions for both assets with one configured issuer wallet, and that is now the confirmed design: the owner has settled that the production $rPND issuance comes from the same account as $PND — [OQ-22](docs/open-questions.md#oq-22). The live deployment's account topology is also settled beyond the single "operational account" this repo used to describe: **Treasury** (`rPNDcL2UrGtSoGwruWx6ocMQ6ey8uPZm2b`) and **Operations** (`rPNDAwFzgXzsjvUbVWz1ErB28v9SkcR2in`) are two separate named accounts — see [OQ-23](docs/open-questions.md#oq-23). Treasury is funded on mainnet (~1 XRP; about 2 XRP without escrow); Operations is not funded (`actNotFound`); a bot-ops account is recommended but not yet created. **Do not treat a 90 billion Treasury escrow (or ten 9 billion self-escrows) as the public schedule.** High-level $PND split: **10 billion public, 10 billion team, 80 billion to holders at 10 billion per month from 2027-01-01 through 2027-08-01, proportional to $PND held.** That is a snapshot plus treasury payments, not TokenEscrow, and not a claim flow. Launch day **2026-10-01** does not start the holder monthly.
...
- **Treasury** — `rPNDcL2UrGtSoGwruWx6ocMQ6ey8uPZm2b`. Named account for the 80 billion holder inventory and the launch team payment. Funded on mainnet; about 2 XRP without escrow. Holder months are snapshot plus treasury payments, **not TokenEscrow** ([OQ-23](docs/open-questions.md#oq-23), [OQ-13](docs/open-questions.md#oq-13)).
- **Operations (hot)** — `rPNDAwFzgXzsjvUbVWz1ErB28v9SkcR2in`. Opens the $PND trust line, authorizes $rPND, holds the 10 billion public slice, and creates the AMM pool. Not funded on ledger yet. Not the monthly drop wallet.
