...
source_ref: worktree
source_sha256: 04d8f5ea10c5a0010bd5b8ed451b4b103beb6ab7a2f2851e2ee5b118cb834314
title: Overview
url: /protocol/
section: Protocol
synced: 2026-09-16
...
The issuance toolkit signs transactions for both assets with one configured issuer wallet, and that is now the confirmed design: the owner has settled that the production $rPND issuance comes from the same account as $PND — [OQ-22](docs/open-questions.md#oq-22). The live deployment's account topology is also settled beyond the single "operational account" this repo used to describe: **Treasury** (`rPNDcL2UrGtSoGwruWx6ocMQ6ey8uPZm2b`) and **Operations** (`rPNDAwFzgXzsjvUbVWz1ErB28v9SkcR2in`) are two separate named accounts — see [OQ-23](docs/open-questions.md#oq-23). Treasury is funded on mainnet (needs more XRP before launch lockups); Operations is not funded (`actNotFound`); a bot-ops account is recommended but not yet created. **Do not treat a 90 billion Treasury escrow (or ten 9 billion self-escrows) as the public schedule.** High-level $PND split, being revised until a vesting doc lands: **10 billion public, 10 billion team, 80 billion to holders at 10 billion per month from 2027-01-01.** That is not signed escrow, not an airdrop, and not a claim flow. Launch day **2026-10-01** does not start the holder monthly.
...
- **Treasury** — `rPNDcL2UrGtSoGwruWx6ocMQ6ey8uPZm2b`. Named account for supply that is not the trading wallet. Funded on mainnet; needs more XRP before launch lockups. How it will hold the team slice and the holder monthly is **not published as escrow** ([OQ-23](docs/open-questions.md#oq-23), [OQ-13](docs/open-questions.md#oq-13)).
- **Operations (hot)** — `rPNDAwFzgXzsjvUbVWz1ErB28v9SkcR2in`. Opens the $PND trust line, authorizes $rPND, holds day-to-day liquidity inventory, and creates the AMM pool. Not funded on ledger yet. Which slice of the 10 billion public it holds is not locked.
- **Bot-ops (recommended, not created)** — any automation gets a dedicated, bounded fifth-role account of its own, never a key on Operations, Treasury, or the Issuer. Operations is the liquidity wallet and must not double as a bot's wallet.
